...
def create_slope_sum(df):
    ...
    close_columns, change_columns = get_columns_with_CLS(columns)

    # get rid of the CLS columns and only run slop on the CHG columns
    df.drop(columns=close_columns, inplace=True)

    column_index = 1

    while column_index < len(change_columns) - 1:

        # calculate the slope between L & R columns then sum them
        slope_sum = df[change_columns[column_index]] - df[change_columns[column_index - 1]] + \
                    df[change_columns[column_index]] - df[change_columns[column_index + 1]]

        # add the newly formed column containing the slope info to the main df
        df[str(change_columns[column_index].replace('CHG', 'slope_sum'))] = slope_sum
...
def get_columns_with_CLS(columns):
    ...
    for column in columns:
        ...
        if column[-3:] == 'CLS':
            ...
            columns_with_CHG.append(column)
    return columns_with_CLS, columns_with_CHG
...
def _sliding_window(sequence, win_size, step=1):
    """Returns a generator that will iterate through
    the defined chunks of input sequence.  Input sequence
    must be iterable.

    NOTE: Good pattern for better memory usage
    """
...
def generate_target_values(df, batch_count, column_name, look_ahead):
    """
    Takes a dataframe and a batch count to generate the % change values good or bad for each batch
    """
...
def create_batch_of_slopes(df, column_with_slope_sum, batch_count, cut_length):
    ...
    list_of_chunks = list(_sliding_window(df[column_with_slope_sum].tolist(), batch_count))
